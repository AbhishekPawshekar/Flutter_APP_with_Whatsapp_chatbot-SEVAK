...
class road_work_headState extends StatefulWidget {
  var admin_email;
  road_work_headState({Key key,@required this.admin_email}):super(key: key);

  @override
  _road_workState_headState createState() => _road_workState_headState(admin_email);
}

class _road_workState_headState extends State<road_work_headState> {
  var admin_email;
  _road_workState_headState(this.admin_email);
  String road_total,road_unsolved,road_solved;
  ...
  get_data_form_firestore_admin()async{
    List data=await get_data_from_dept_head(request_choice, 'area', admin_email,'dept','road');
    length_of_road_member=await data[0];
    road_total=await data[2];
    road_solved= await data[3];
    road_unsolved=await data[4];
    return await data[1];
  }
  getchartdata(){
    Map<String,double> pie_data={"Total Request":double.parse(road_total),
      "Solved Request":double.parse(road_solved),
      "Unsolved Request":double.parse(road_unsolved)};
    return pie_data;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      Navigator.push(context, right_to_left_transition(child: Login_page(),),);
        ...
        ,backgroundColor: Colors.transparent,centerTitle: true,title: Text('Road Works',style: TextStyle(color: Colors.orangeAccent,),),),
      body: FutureBuilder(
          ...
          if(snapshot.connectionState==ConnectionState.done){
            if(snapshot.hasError) {
              return AlertDialog(title: const Text('Error:-',
                ...
                content: Text(
                  '${snapshot.error}', style: TextStyle(fontSize: 12),),
                actions: <Widget>[
                  TextButton(
                  ...
                  children: <Widget>[
                    ...
                    Row(mainAxisAlignment:MainAxisAlignment.spaceAround,
                      children:<Widget> [
                        Column(
                          children: [
                            Text('$road_total',style: TextStyle(color: Colors.blueAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$road_solved',style: TextStyle(color: Colors.green,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$road_unsolved',style: TextStyle(color: Colors.redAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                          ...
                          Padding(padding: EdgeInsets.only(left: 10,top: 15,right: 10),
                            child: Row(
                              ...
                              children: <Widget>[
                                ...
                                Row(children: <Widget>[
                                  Text('$road_total',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.blue),),
                                  SizedBox(width: 10,),
                                  Text('$road_solved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.green),),
                                  SizedBox(width: 10,),
                                  Text('$road_unsolved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.red),),
                                ],),
                              ],
                            ),),
                          SizedBox(height: 10.0,),
                          SizedBox(height: MediaQuery.of(context).size.height/2,
                            child: ListView.builder(itemCount: length_of_road_member,itemBuilder: (BuildContext cntx,int index){
