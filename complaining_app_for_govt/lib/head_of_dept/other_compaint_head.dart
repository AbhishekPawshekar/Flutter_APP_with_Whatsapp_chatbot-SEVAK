...
class other_compaint_headState extends StatefulWidget {
var admin_email;
  other_compaint_headState({Key key,@required this.admin_email}):super(key: key);

  @override
  _other_compaintState_headState createState() => _other_compaintState_headState(admin_email);
}

class _other_compaintState_headState extends State<other_compaint_headState> {
  var admin_email;
  _other_compaintState_headState(this.admin_email);
  String other_total,other_solved,other_unsolved;
  ...
  get_data_form_firestore_admin()async{
    List data=await get_data_from_dept_head(request_choice, 'area', admin_email,'dept','other');
    length=await data[0];
    other_total=await data[2];
    other_solved= await data[3];
    other_unsolved=await data[4];
    return await data[1];
  }

getchartdata(){
  Map<String,double> pie_data={"Total Request":double.parse(other_total),
    "Solved Request":double.parse(other_solved),
    "Unsolved Request":double.parse(other_unsolved)};
  return pie_data;
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      Navigator.push(context, right_to_left_transition(child: Login_page(),),);
        ...
        ,backgroundColor: Colors.transparent,centerTitle: true,title: Text('Other',style: TextStyle(color: Colors.orangeAccent,),),),
      body: FutureBuilder(
                  ...
                  children: <Widget>[
                    ...
                    Row(mainAxisAlignment:MainAxisAlignment.spaceAround,
                      children:<Widget> [
                        Column(
                          children: [
                            Text('$other_total',style: TextStyle(color: Colors.blueAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$other_solved',style: TextStyle(color: Colors.green,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$other_unsolved',style: TextStyle(color: Colors.redAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                          ...
                          Padding(padding: EdgeInsets.only(left: 10,top: 15,right: 10),
                            child: Row(
                              ...
                              children: <Widget>[
                                ...
                                Row(children: <Widget>[
                                  Text('$other_total',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.blue),),
                                  SizedBox(width: 10,),
                                  Text('$other_solved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.green),),
                                  SizedBox(width: 10,),
                                  Text('$other_unsolved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.red),),
                                ],),
                              ],
                            ),),
