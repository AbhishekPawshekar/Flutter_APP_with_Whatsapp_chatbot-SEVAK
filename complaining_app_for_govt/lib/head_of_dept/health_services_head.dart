...
class health_services_headState extends StatefulWidget {
var admin_email;
  health_services_headState({Key key,@required this.admin_email}):super(key: key);


  @override
  _health_servicesState_headState createState() => _health_servicesState_headState(admin_email);
}

class _health_servicesState_headState extends State<health_services_headState> {
  var admin_email;
  _health_servicesState_headState(this.admin_email);
  String health_total,health_unsolved,health_solved;
  ...
  get_data_form_firestore_admin()async{
    List data=await get_data_from_dept_head(request_choice, 'area', admin_email,'dept','health');
    length=await data[0];
    health_total=await data[2];
    health_solved= await data[3];
    health_unsolved=await data[4];
    return await data[1];

  }
  getchartdata(){
    Map<String,double> pie_data={"Total Request":double.parse(health_total),
      "Solved Request":double.parse(health_solved),
      "Unsolved Request":double.parse(health_unsolved)};
    return pie_data;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      Navigator.push(context, right_to_left_transition(child: Login_page(),),);
        ...
        ,backgroundColor: Colors.transparent,centerTitle: true,title: Text('Health Services',style: TextStyle(color: Colors.orangeAccent,),),),
      body:FutureBuilder(
                  ...
                  children: <Widget>[
                    ...
                    Row(mainAxisAlignment:MainAxisAlignment.spaceAround,
                      children:<Widget> [
                        Column(
                          children: [
                            Text('$health_total',style: TextStyle(color: Colors.blueAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$health_solved',style: TextStyle(color: Colors.green,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$health_unsolved',style: TextStyle(color: Colors.redAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                          ...
                          Padding(padding: EdgeInsets.only(left: 10,top: 15,right: 10),
                            child: Row(
                              ...
                              children: <Widget>[
                                ...
                                Row(children: <Widget>[
                                  Text('$health_total',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.blue),),
                                  SizedBox(width: 10,),
                                  Text('$health_solved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.green),),
                                  SizedBox(width: 10,),
                                  Text('$health_unsolved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.red),),
                                ],),
                              ],
                            ),),
