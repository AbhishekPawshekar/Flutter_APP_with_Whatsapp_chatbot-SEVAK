...
class electric_work_headState extends StatefulWidget {
  var admin_email;
  electric_work_headState({Key key,@required admin_email}):super(key: key);


  @override
  _electric_workState_headState createState() => _electric_workState_headState(admin_email);
}

class _electric_workState_headState extends State<electric_work_headState> {
  var admin_email;
  _electric_workState_headState(this.admin_email);
  ...
  String electric_total,electric_unsolved,electric_solved;
  ...
  get_data_form_firestore_admin()async{
    List data=await get_data_from_dept_head(request_choice, 'area', admin_email,'dept','electric');
    length=await data[0];
    electric_total=await data[2];
    electric_solved= await data[3];
    electric_unsolved=await data[4];
    return await data[1];
  }
  getchartdata(){
    Map<String,double> pie_data={"Total Request":double.parse(electric_total),
      "Solved Request":double.parse(electric_solved),
      "Unsolved Request":double.parse(electric_unsolved)};
    return pie_data;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      Navigator.push(context, right_to_left_transition(child: Login_page(),),);
        ...
        ,backgroundColor: Colors.transparent,centerTitle: true,title: Text('Electric Work',style: TextStyle(color: Colors.orangeAccent,),),),
      body: FutureBuilder(
                  ...
                  children: <Widget>[
                    ...
                    Row(mainAxisAlignment:MainAxisAlignment.spaceAround,
                      children:<Widget> [
                        Column(
                          children: [
                            Text('$electric_total',style: TextStyle(color: Colors.blueAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$electric_solved',style: TextStyle(color: Colors.green,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$electric_unsolved',style: TextStyle(color: Colors.redAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                          ...
                          Padding(padding: EdgeInsets.only(left: 10,top: 15,right: 10),
                            child: Row(
                              ...
                              children: <Widget>[
                                ...
                                Row(children: <Widget>[
                                  Text('$electric_total',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.blue),),
                                  SizedBox(width: 10,),
                                  Text('$electric_solved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.green),),
                                  SizedBox(width: 10,),
                                  Text('$electric_unsolved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.red),),
                                ],),
                              ],
                            ),),
