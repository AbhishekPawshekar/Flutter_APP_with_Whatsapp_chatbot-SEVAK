...
class sewage_and_sanitation_headState extends StatefulWidget {
  var admin_email;
  sewage_and_sanitation_headState({Key key,@required this.admin_email}):super(key: key);


  @override
  _sewage_and_sanitationState_headState createState() => _sewage_and_sanitationState_headState(admin_email);
}

class _sewage_and_sanitationState_headState extends State<sewage_and_sanitation_headState> {
  var admin_email;
  _sewage_and_sanitationState_headState(this.admin_email);
  String sewage_total,sewage_unsolved,sewage_solved;
  ...
  get_data_form_firestore_admin()async{
    List data=await get_data_from_dept_head(request_choice, 'area', admin_email,'dept','sewage');
    length=await data[0];
    sewage_total=await data[2];
    sewage_solved= await data[3];
    sewage_unsolved=await data[4];
    return await data[1];
  }
  getchartdata(){
    Map<String,double> pie_data={"Total Request":double.parse(sewage_total),
      "Solved Request":double.parse(sewage_solved),
      "Unsolved Request":double.parse(sewage_unsolved)};
    return pie_data;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      Navigator.push(context, right_to_left_transition(child: Login_page(),),);
        ...
        ,backgroundColor: Colors.transparent,centerTitle: true,title: Text('Sewage And Sanitation',style: TextStyle(color: Colors.orangeAccent,),),),
      body:FutureBuilder(
                  ...
                  children: <Widget>[
                    ...
                    Row(mainAxisAlignment:MainAxisAlignment.spaceAround,
                      children:<Widget> [
                        Column(
                          children: [
                            Text('$sewage_total',style: TextStyle(color: Colors.blueAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$sewage_solved',style: TextStyle(color: Colors.green,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                        ...
                        Column(
                          children: [
                            Text('$sewage_unsolved',style: TextStyle(color: Colors.redAccent,fontSize: 14,fontWeight: FontWeight.bold,fontFamily: 'abrifatface'),),
                            SizedBox(height: 10,),
                          ...
                          Padding(padding: EdgeInsets.only(left: 10,top: 15,right: 10),
                            child: Row(
                              ...
                              children: <Widget>[
                                ...
                                Row(children: <Widget>[
                                  Text('$sewage_total',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.blue),),
                                  SizedBox(width: 10,),
                                  Text('$sewage_solved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.green),),
                                  SizedBox(width: 10,),
                                  Text('$sewage_unsolved',style: TextStyle(fontSize: 12,fontWeight: FontWeight.bold,color: Colors.red),),
                                ],),
                              ],
                            ),),
